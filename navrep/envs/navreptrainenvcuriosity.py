...
class NavRepTrainEnvCuriosity(gym.Env):
    ...
    def __init__(self, scenario='test', silent=False, legacy_mode=False, adaptive=True, lidar_legs=True,
        ...
        # gym env definition
        super(NavRepTrainEnvCuriosity, self).__init__()
    ...
    def step(self, action):
        ...
        if done and self.adaptive and self.scenario == "train":
            if isinstance(info, ReachGoal):
                ...
                #if self.reached_goal_counter >= 2 :                        
                #    self.soadrl_sim.human_num += 1
                #    self.soadrl_sim.num_walls += 1
                #    self.soadrl_sim.num_circles += 1
                #    self.reached_goal_counter = 0
                ...
                # set robot to other random position on map
                #self.soadrl_sim.robot.px = 0
                #self.soadrl_sim.robot.py = 0
                #self.soadrl_sim.robot.theta = 0
                self.reset()
                done = False
                #print("reset done signal false")
                #print(self.reached_goal_counter)
