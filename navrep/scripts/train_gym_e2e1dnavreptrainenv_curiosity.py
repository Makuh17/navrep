...
if __name__ == "__main__":
    ...
    LOGNAME = "CURIOSITY_e2e1dnavreptrainenv_" + \
        START_TIME + "_PPO" + "_E2E1D" + CONTROLLER_ARCH
    LOGPATH = os.path.join(LOGDIR, LOGNAME + ".csv")
    # TODO also need to save curiosiry reward model
    ...
    if args.debug:
        env = DummyVecEnv([lambda: CuriosityWrapper(
            E2E1DNavRepEnv(silent=True, scenario='train'))]*N_ENVS)
        # env = DummyVecEnv([lambda: E2E1DNavRepEnv(silent=True, scenario='train')]*N_ENVS)
    else:
        env = SubprocVecEnv([lambda: CuriosityWrapper(E2E1DNavRepEnv(silent=True, scenario='train'))]*N_ENVS,
                            start_method='spawn')
